...
public class TcpSupremeMQServerTransport implements SupremeMQServerTransport {
	...
	private byte[] objectByte = new byte[com.suprememq.message.Message.OBJECT_BYTE_SIZE];
	
	private SupremeMQTransportCenter tcpSupremeMQTransportCenter;
	
	private Logger logger = LoggerFactory.getLogger(TcpSupremeMQServerTransport.class);
	
	public TcpSupremeMQServerTransport(Socket socket, SupremeMQTransportCenter tcpSupremeMQTransportCenter) {
		if(socket == null) {
	...
	public void start() throws JMSException {
		if(socket.isClosed()) {
			logger.error("Socket已经关闭，TcpSupremeMQServerTransport启动失败！");
			throw new JMSException("Socket已经关闭，TcpSupremeMQServerTransport启动失败！");
		}
		
		if(!socket.isConnected()) {
			logger.error("Socket未连接，TcpSupremeMQServerTransport启动失败！");
			throw new JMSException("Socket未连接，TcpSupremeMQServerTransport启动失败！");
		}
		...
		if(!socket.isOutputShutdown()) {
			...
			logger.debug("Socket未连接，TcpSupremeMQServerTransport开启消息发送线程失败！");
		}
	}
	...
	public void close() {
		synchronized (isClosed) {
			...
			logger.debug("TcpSupremeMQServerTransport即将被关闭！");
			...
			logger.debug("TcpSupremeMQServerTransport已被关闭！");
		}
	}
	...
	private void receiveMessage() {
		try {
			...
			logger.error("Socket状态异常，TcpSupremeMQServerTransport消息接收线程结束！");
		} catch (Exception e) {
	...
	private void sendMessage() {
		...
		while(true) {
			try {
				...
				logger.info("TcpSupremeMQServerTransport消息发送线程被要求停止！");
				break ;
			}
		...
		logger.info("TcpSupremeMQServerTransport消息发送线程结束！");
	}
